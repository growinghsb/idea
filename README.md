...
* 현재 내가 진행하고 있는 식단 or 진행하고 싶은 식단등이 있으면      
  ...
  2차 : 식단을 하는 사람들을 위한 어플이다. 내가 식단 하려고 하는 음식에 대한     
  정보를 배달 가능한 식당을 기준으로 알려주고, 그 음식을 시켜 먹으면 자동적으로    
  내가 먹은 것에 대한 정보가 기입된다. 이렇게 되면 매번 내가 먹은 것을 기입하지     
  않아도 된다. 만약 내가 식단 하려는 매뉴가 없다면 레시피와 그 레시피를 구현할 수     
  있도록 상품 정보를 띄워주게 된다. 식단을 정하고, 상품을 정하면 그 상품을 바로      
  주문할 수 있는 사이트로 연결해 주는 것이다. 쿠팡 파트너스?????ㅋㅋㅋㅋㅋㅋ        
  배달음식도 추천 하고, 구매 사이트도 추천 하는 것이다. 아마 쿠팡하고 스마트스토어면     
  대부분 커버 되지 싶다.
***
### 내가 낭비한 시간    
